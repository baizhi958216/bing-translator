...
### 新增
- **动态语言选择**：用户现在可以在翻译时选择源语言和目标语言，而不仅仅依赖设置
- **统一的 Webview 翻译界面**：全新的现代化翻译面板，所有选项集中在一个界面
  - 源语言和目标语言下拉选择框
  - 语言交换按钮，快速切换源语言和目标语言
  - 支持多行的文本输入区域
  - 翻译结果显示区域
  - 复制结果和替换选中文本按钮
  - Ctrl+Enter 快捷键快速翻译
- **界面模式配置**：新增 `useWebviewUI` 设置，可在两种界面模式间切换
  - Webview 模式（默认）：统一界面，所有选项在一个面板中
  - QuickPick 模式：多步骤界面，通过独立对话框选择语言
- **语言列表模块**：添加了包含 30+ 种常用语言的完整列表
  - 支持中英文语言名称显示
  - 源语言支持自动检测选项
...
- 增强的国际化支持
  - 所有用户可见的字符串现已正确本地化
  - 修复了 commands.js 中的硬编码英文字符串
  - 在 i18n 模块中添加了语言选择提示
- 通过灵活的翻译工作流程提升用户体验
- 根据 VS Code 语言设置自动显示语言名称
